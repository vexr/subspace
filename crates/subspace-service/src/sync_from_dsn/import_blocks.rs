    ...
    IQS: ImportQueueService<Block> + ?Sized,
{
    ...
    while let Some(segment_index) = segment_indices_iter.next() {
        ...
        // import
        if let Some(last_partial_archived_block_number) = last_partial_archived_block_number
            && last_partial_archived_block_number == *last_processed_block_number + One::one()
